  ...
  return (
    <nav className='fixed bottom-0 mx-auto w-[598px] max-w-[597px] border-t-2 bg-[#fff]'>
      <ul className='flex justify-around p-4'>
        <BottomNavLinkComp router='/' icons={HomeIcon} actIcons={HomeActIcons} pageName={'홈'} />
        <BottomNavLinkComp router='/products' icons={ItemsIcon} actIcons={ItemsActIcons} pageName={'상품'} />
        <BottomNavLinkComp router='/wishlist' icons={WishIcon} actIcons={WishActIcons} pageName={'관심물품'} />
        <BottomNavLinkComp router='/used' icons={UsedIcon} actIcons={UsedActIcon} pageName={'중고거래'} />
        <BottomNavLinkComp router='/my' icons={MyIcon} actIcons={MyActIcon} pageName={'마이'} />
      </ul>
    </nav>
  );
};
